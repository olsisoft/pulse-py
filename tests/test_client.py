...
class TestConnectors:
    """B-093 follow-up — client.connectors (catalogue parity with the UI/CLI)."""

    @respx.mock
    def test_list_returns_sinks_and_sources(
        self, authed_client: PulseClient, base_url: str
    ) -> None:
        respx.get(f"{base_url}/api/pulse/connectors").mock(
            return_value=httpx.Response(
                200,
                json={
                    "sinks": [
                        {
                            "subType": "segment",
                            "displayName": "Segment",
                            "configFields": [{"key": "segment.write.key", "type": "password"}],
                        }
                    ],
                    "sources": [
                        {"subType": "posthog-source", "displayName": "PostHog Source (poll)", "configFields": []}
                    ],
                },
            )
        )
        catalog = authed_client.connectors.list()
        assert catalog["sinks"][0]["subType"] == "segment"
        assert catalog["sources"][0]["subType"] == "posthog-source"

    @respx.mock
    def test_sinks_and_sources_helpers(
        self, authed_client: PulseClient, base_url: str
    ) -> None:
        respx.get(f"{base_url}/api/pulse/connectors").mock(
            return_value=httpx.Response(
                200,
                json={
                    "sinks": [{"subType": "amplitude", "displayName": "Amplitude", "configFields": []}],
                    "sources": [{"subType": "posthog-source", "displayName": "PostHog Source (poll)", "configFields": []}],
                },
            )
        )
        assert authed_client.connectors.sinks()[0]["subType"] == "amplitude"
        assert authed_client.connectors.sources()[0]["subType"] == "posthog-source"

    @respx.mock
    def test_missing_keys_degrade_to_empty(
        self, authed_client: PulseClient, base_url: str
    ) -> None:
        respx.get(f"{base_url}/api/pulse/connectors").mock(
            return_value=httpx.Response(200, json={})
        )
        assert authed_client.connectors.sinks() == []
        assert authed_client.connectors.sources() == []
